...
TEST_SUITE("DC Bus Limiter") {
    ...
    TEST_CASE("derates on the motoring power cap") {
        ...
        const auto           s = lim.evaluate(DQ{.q = 20.0}, DQ{.q = 10.0}, 48.0); // 300 W
    ...
    TEST_CASE("derates on the motoring current cap") {
        ...
        const auto           s = lim.evaluate(DQ{.q = 20.0}, DQ{.q = 10.0}, 48.0); // I_bus = 6.25 A
    ...
    TEST_CASE("under/over-voltage raises the disarm gate") {
        ...
        CHECK_FALSE(lim.evaluate(DQ{.q = 1.0}, DQ{.q = 1.0}, 10.0).ok); // UV
        CHECK_FALSE(lim.evaluate(DQ{.q = 1.0}, DQ{.q = 1.0}, 60.0).ok); // OV
        CHECK(lim.evaluate(DQ{.q = 1.0}, DQ{.q = 1.0}, 48.0).ok);       // in range
    }

    TEST_CASE("unbounded defaults never derate") {
        DcBusLimiter<double> lim{};
        const auto           s = lim.evaluate(DQ{.q = 100.0}, DQ{.q = 500.0}, 48.0);
