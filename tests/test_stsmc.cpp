...
TEST_SUITE("stsmc") {
    TEST_CASE("synthesize_stsmc computes Levant/Moreno gains and validates") {
        SUBCASE("valid: k1 = 1.5*sqrt(L), k2 = 1.1*L") {
            const auto r = design::synthesize_stsmc(4.0);
            CHECK(r.success);
            CHECK(r.k1 == doctest::Approx(1.5 * std::sqrt(4.0)));
            CHECK(r.k2 == doctest::Approx(1.1 * 4.0));
        }
        SUBCASE("gain_margin scales both gains") {
            const auto r = design::synthesize_stsmc(4.0, 0.0, 0.0, 0.0, 2.0);
            CHECK(r.k1 == doctest::Approx(2.0 * 1.5 * std::sqrt(4.0)));
            CHECK(r.k2 == doctest::Approx(2.0 * 1.1 * 4.0));
        }
        SUBCASE("rejects bad specs") {
            CHECK_FALSE(design::synthesize_stsmc(0.0).success);                     // L <= 0
            CHECK_FALSE(design::synthesize_stsmc(4.0, 0.0, -1.0).success);          // kl < 0
            CHECK_FALSE(design::synthesize_stsmc(4.0, 0.0, 0.0, 0.0, 0.5).success); // margin < 1
        }
        SUBCASE("direct-gain factory validates") {
            CHECK(design::stsmc(2.0, 2.2).success);
            CHECK_FALSE(design::stsmc(-1.0, 2.2).success);
            CHECK_FALSE(design::stsmc(2.0, 0.0).success);
        }
    }
    ...
    TEST_CASE("classic super-twisting rejects a Lipschitz disturbance, continuously") {
        ...
        const double L = 2.0;
        const auto   art = design::synthesize_stsmc(L);
        REQUIRE(art.success);
    ...
    TEST_CASE("first-order SMC chatters where super-twisting does not (the whole point)") {
        ...
        SuperTwistingController<double> st(design::synthesize_stsmc(L));
        const auto [st_s, st_du] = run(st, d, Ts, 8000, 1.0);

        // A bare sign()-based controller of comparable authority: u = -k*sign(s).
        struct SignSMC {
            double k;
            double control(double s, double /*Ts*/) { return -k * static_cast<double>(wet::sgn(s)); }
        } sign_smc{3.0};
    ...
    TEST_CASE("generalized STA (kl > 0) also converges") {
        const double Ts = 1e-3;
        const double L = 2.0;
        const auto   art = design::synthesize_stsmc(L, 0.0, /*kl=*/1.5);
        REQUIRE(art.success);
    ...
    TEST_CASE("boundary layer keeps the canonical loop bounded and continuous") {
        const double Ts = 1e-3;
        const auto   art = design::synthesize_stsmc(2.0, 0.0, 0.0, /*epsilon=*/0.05);
        REQUIRE(art.success);
        SuperTwistingController<double> c(art);
    ...
    TEST_CASE("invalid design is inert; reset clears state") {
        SuperTwistingController<double> bad(design::synthesize_stsmc(-1.0));
        CHECK_FALSE(bad.valid());
        CHECK(bad.control(5.0, 1e-3) == doctest::Approx(0.0));

        SuperTwistingController<double> c(design::synthesize_stsmc(2.0));
        (void)c.control(1.0, 1e-3);
        (void)c.control(1.0, 1e-3);
    ...
    TEST_CASE("float deployment via as<float>()") {
        const auto                     art = design::synthesize_stsmc(2.0);
        SuperTwistingController<float> c(art.as<float>());
        const float                    u = c.control(1.0f, 1e-3f);
    ...
    TEST_CASE("super-twisting controller is constexpr-evaluable") {
        constexpr double final_s = []() consteval {
            auto                            art = design::synthesize_stsmc(2.0);
            SuperTwistingController<double> c(art);
            double                          s = 1.0;
            for (int k = 0; k < 4000; ++k) {
                const double u = c.control(s, 1e-3);
