...
template <typename T>
concept ODEIntegrator = requires(T integrator, ODEFunction f, const Matrix& x, double t, double h) {
    { integrator.evolve(f, x, t, h) } -> std::same_as<IntegrationResult>;
};

// Concept for integrators that support LTI systems
template <typename T>
concept LTIIntegrator = requires(T integrator, const Matrix& A, const Matrix& B, const Matrix& x, const Matrix& u, double h) {
...
struct IntegratorBase {
   public:
    template <typename F>
    IntegrationResult evolve(F&& f, const Matrix& x, double t, double h) const {
        return static_cast<const Derived*>(this)->evolve(std::forward<F>(f), x, t, h);
    }

    IntegrationResult evolve(const Matrix& A, const Matrix& B, const Matrix& x, const Matrix& u, double h) const {
...
struct Discrete : public IntegratorBase<Discrete> {
    ...
    static constexpr bool is_stiff         = false;

    IntegrationResult evolve(const Matrix& A, const Matrix& B, const Matrix& x, const Matrix& u) const {
        return {A * x + B * u, 0.0};
    }
...
struct BackwardEuler : public IntegratorBase<BackwardEuler> {
    ...
    static constexpr bool is_adaptive_step = false;
    static constexpr bool is_exact         = false;
    static constexpr bool is_stiff         = true;  // L-stable, good for stiff systems
...
struct BDF2 : public IntegratorBase<BDF2> {
    ...
    static constexpr bool is_adaptive_step = false;
    static constexpr bool is_exact         = false;
    static constexpr bool is_stiff         = true;  // Excellent for stiff systems
    ...
    IntegrationResult evolve(const Matrix& A, const Matrix& B, const Matrix& x, const Matrix& u, double h) const {
        ...
        Matrix lhs    = I - c2 * h * A;
        Matrix rhs    = c0 * x + c1 * x_prev + c2 * h * B * u;
        Matrix x_next = lhs.inverse() * rhs;
    ...
    IntegrationResult evolve(F&& f, const Matrix& x, double t, double h) const {
        ...
        if (first_step) {
            // Use Backward Euler for first step
            Matrix y = x + h * f(t, x);  // Initial guess
            for (size_t i = 0; i < max_iter; ++i) {
                Matrix y_next = x + h * f(t + h, y);
                if ((y_next - y).norm() <= tol) {
                    y = y_next;
        ...
        Matrix y = c0 * x + c1 * x_prev;  // Initial guess without implicit term

        for (size_t i = 0; i < max_iter; ++i) {
            Matrix y_next = c0 * x + c1 * x_prev + c2 * h * f(t + h, y);
            if ((y_next - y).norm() <= tol) {
                y = y_next;
                break;
            }
   ...
   private:
    mutable Matrix x_prev;      // Previous state (for multistep)
...
struct Trapezoidal : public IntegratorBase<Trapezoidal> {
    ...
    static constexpr bool is_adaptive_step = false;
    static constexpr bool is_exact         = false;
    static constexpr bool is_stiff         = true;  // A-stable, good for moderately stiff systems

    IntegrationResult evolve(const Matrix& A, const Matrix& B, const Matrix& x, const Matrix& u, double h) const {
        Matrix I      = Matrix::Identity(A.rows(), A.cols());
        Matrix M      = (I - 0.5 * h * A).inverse();
        Matrix x_next = M * ((I + 0.5 * h * A) * x + h * B * u);
        return {x_next, 0.0};
    }
...
struct RK4 : public IntegratorBase<RK4> {
    ...
    IntegrationResult evolve(const Matrix& A, const Matrix& B, const Matrix& x, const Matrix& u, double h) const {
        // RK4 for LTI system
        auto f = [&](double, const Matrix& x) { return A * x + B * u; };
    ...
    IntegrationResult evolve(F&& f, const Matrix& x, double t, double h) const {
        Matrix k1     = f(t, x);
        Matrix k2     = f(t + 0.5 * h, x + 0.5 * h * k1);
        Matrix k3     = f(t + 0.5 * h, x + 0.5 * h * k2);
        Matrix k4     = f(t + h, x + h * k3);
        Matrix x_next = x + (h / 6.0) * (k1 + 2 * k2 + 2 * k3 + k4);
        return {x_next, 0.0};
    }
};
...
struct RK45 : public IntegratorBase<RK45> {
    ...
    IntegrationResult evolve(F&& f, const Matrix& x, double t, double h) const {
        ...
        Matrix k1 = f(t, x);
        Matrix k2 = f(t + h * c2, x + h * a21 * k1);
        Matrix k3 = f(t + h * c3, x + h * (a31 * k1 + a32 * k2));
        Matrix k4 = f(t + h * c4, x + h * (a41 * k1 + a42 * k2 + a43 * k3));
        Matrix k5 = f(t + h, x + h * (a51 * k1 + a52 * k2 + a53 * k3 + a54 * k4));
        Matrix k6 = f(t + h * c6, x + h * (a61 * k1 + a62 * k2 + a63 * k3 + a64 * k4 + a65 * k5));

        Matrix x4 = x + h * (b41 * k1 + b43 * k3 + b44 * k4 + b45 * k5);
        Matrix x5 = x + h * (b51 * k1 + b53 * k3 + b54 * k4 + b55 * k5 + b56 * k6);
...
struct RK23 : public IntegratorBase<RK23> {
    ...
    template <typename F>
    IntegrationResult evolve(F&& f, const Matrix& x, double t, double h) const {
        // Bogacki-Shampine (2,3) pair - MATLAB's ode23
        ...
        Matrix k1 = f(t, x);
        Matrix k2 = f(t + c2 * h, x + h * a21 * k1);
        Matrix k3 = f(t + c3 * h, x + h * a32 * k2);

        Matrix x3 = x + h * (b1 * k1 + b2 * k2 + b3 * k3);
        Matrix k4 = f(t + h, x3);

        // 2nd order solution (for error estimate)
        Matrix x2 = x + h * (e1 * k1 + e2 * k2 + e3 * k3 + e4 * k4);
...
struct RK3 : public IntegratorBase<RK3> {
    ...
    IntegrationResult evolve(F&& f, const Matrix& x, double t, double h) const {
        ...
        Matrix k1 = f(t, x);
        Matrix k2 = f(t + c2 * h, x + h * a21 * k1);
        Matrix k3 = f(t + h, x + h * (a31 * k1 + a32 * k2));
...
struct DP5 : public IntegratorBase<DP5> {
    ...
    template <typename F>
    IntegrationResult evolve(F&& f, const Matrix& x, double t, double h) const {
        // Dormand-Prince 5th order (fixed-step, MATLAB ode5)
        ...
        Matrix k1 = f(t, x);
        Matrix k2 = f(t + c2 * h, x + h * a21 * k1);
        Matrix k3 = f(t + c3 * h, x + h * (a31 * k1 + a32 * k2));
        Matrix k4 = f(t + c4 * h, x + h * (a41 * k1 + a42 * k2 + a43 * k3));
        Matrix k5 = f(t + c5 * h, x + h * (a51 * k1 + a52 * k2 + a53 * k3 + a54 * k4));
        Matrix k6 = f(t + h, x + h * (a61 * k1 + a62 * k2 + a63 * k3 + a64 * k4 + a65 * k5));

        Matrix x_next = x + h * (b1 * k1 + b3 * k3 + b4 * k4 + b5 * k5 + b6 * k6);
        return {x_next, 0.0};
    }
};

struct Integrator : public std::variant<Discrete, ForwardEuler, BackwardEuler, Trapezoidal, BDF2, Heun, RK3, RK23, RK4, RK45, DP5> {
