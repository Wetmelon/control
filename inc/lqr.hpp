...
public:
    ...
    // Compile-time only constructor for design:: results
    consteval LQI(const design::LQIResult<NX, NU, NY, T>& result) : Kx(result.Kx), Ki(result.Ki) {}

    // Runtime constructor for online:: results
    constexpr LQI(const online::LQIResult<NX, NU, NY, T>& result) : Kx(result.Kx), Ki(result.Ki) {}
