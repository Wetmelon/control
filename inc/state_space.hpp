...
/**
 * @brief Helper function for default noise matrices
 */
...
struct StateSpace {
    ...
    Matrix<NX, NW, T> G = default_noise_matrix<NX, NW, T>(); //!< Process noise input matrix
    Matrix<NY, NV, T> H = default_noise_matrix<NY, NV, T>(); //!< Measurement noise input matrix
...
/**
 * @brief Evaluate frequency response of state-space system
 *
 * For continuous-time systems: G(s) = C (s I - A)^{-1} B + D
 * For discrete-time systems: G(z) = C (z I - A)^{-1} B + D
 *
 * @param sys State-space system
 * @param s Complex frequency (s for continuous, z for discrete)
 * @return Transfer function matrix G(s) or G(z)
 */
