...
/**
 * @defgroup workflow Workflow Glue APIs
...
/**
 * @brief Workflow artifact bundle for SISO LQG + PR
 */
template<size_t NX, size_t NW = NX, size_t NV = 1, typename T = double, typename TRuntime = float>
struct LQGPRWorkflowArtifacts {
    LQGWorkflowArtifacts<NX, 1, 1, NW, NV, T, TRuntime> base{};
...
/**
 * @brief Workflow artifact bundle: LQGI servo design + analysis + runtime bundle
 */
template<size_t NX, size_t NU, size_t NY, size_t NW = NX, size_t NV = NY, typename T = double, typename TRuntime = float>
struct LQGIWorkflowArtifacts {
    design::LQGIResult<NX, NU, NY, NW, NV, T>            design{};
...
/**
 * @brief Synthesize full LQG workflow artifacts in one call
 */
template<size_t NX, size_t NU, size_t NY, size_t NW = NX, size_t NV = NY, typename T = double, typename TRuntime = float>
[[nodiscard]] constexpr LQGWorkflowArtifacts<NX, NU, NY, NW, NV, T, TRuntime> synthesize_lqg_workflow(
    const StateSpace<NX, NU, NY, NW, NV, T>& sys,
    ...
    LQGWorkflowArtifacts<NX, NU, NY, NW, NV, T, TRuntime> out{};
...
/**
 * @brief Synthesize SISO LQG workflow with PR internal model compensation
 */
template<size_t NX, size_t NW = NX, size_t NV = 1, typename T = double, typename TRuntime = float>
[[nodiscard]] constexpr LQGPRWorkflowArtifacts<NX, NW, NV, T, TRuntime> synthesize_lqg_pr_workflow(
    ...
    LQGPRWorkflowArtifacts<NX, NW, NV, T, TRuntime> out{};

    out.base = synthesize_lqg_workflow<NX, 1, 1, NW, NV, T, TRuntime>(sys, Q_lqr, R_lqr, Q_kf, R_kf, N);
...
/**
 * @brief Synthesize full LQI servo workflow artifacts in one call
 */
template<size_t NX, size_t NU, size_t NY, size_t NW = 0, size_t NV = 0, typename T = double, typename TRuntime = float>
[[nodiscard]] constexpr LQIWorkflowArtifacts<NX, NU, NY, NW, NV, T, TRuntime> synthesize_lqi_workflow(
    const StateSpace<NX, NU, NY, NW, NV, T>& sys,
    const Matrix<NX + NY, NX + NY, T>&       Q_aug,
    const Matrix<NU, NU, T>&                 R
) {
    LQIWorkflowArtifacts<NX, NU, NY, NW, NV, T, TRuntime> out{};
...
/**
 * @brief Synthesize full LQGI servo workflow artifacts in one call
 */
template<size_t NX, size_t NU, size_t NY, size_t NW = NX, size_t NV = NY, typename T = double, typename TRuntime = float>
[[nodiscard]] constexpr LQGIWorkflowArtifacts<NX, NU, NY, NW, NV, T, TRuntime> synthesize_lqgi_workflow(
    const StateSpace<NX, NU, NY, NW, NV, T>& sys,
    const Matrix<NX + NY, NX + NY, T>&       Q_aug,
    const Matrix<NU, NU, T>&                 R,
    const Matrix<NW, NW, T>&                 Q_kf,
    ...
    LQGIWorkflowArtifacts<NX, NU, NY, NW, NV, T, TRuntime> out{};
