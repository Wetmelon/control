...
class RotorObserver {
public:
    constexpr RotorObserver() = default;

    constexpr explicit RotorObserver(const RotorObserverConfig<T>& config)
        : theta_(wrap(config.x0[0], -pi(), pi())), omega_(config.x0[1]), theta_prev_(theta_) {
    ...
    constexpr void predict(T dt) {
        theta_ += omega_ * dt;
        wrap_and_count();
    }

    /// Correct from a wrapped encoder angle in [-π,π) [rad] sampled @p dt [s] ago.
    constexpr void update_encoder(T theta_mech, T dt) {
...
private:
    ...
    T theta_{T{0}};             //!< wrapped mechanical angle [-π,π)
    T omega_{T{0}};             //!< mechanical speed
