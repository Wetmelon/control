...
// ===== Multi-waypoint splines (C² / C³) — a Family-1 extension ===============
//
// Interpolate a list of waypoints with one piecewise polynomial per interval,
// joined so the whole curve is smooth. Cubic segments give C² (continuous
// acceleration); quintic segments give C⁴ (continuous jerk *and* snap — the
// "C³-and-better" smooth option). Solved as one global linear system in all the
// segment coefficients (interpolation at every knot + derivative-continuity at
// the interior knots + boundary conditions), reusing @ref mat::solve — the
// multi-segment generalization of @ref design::synthesize_poly_trajectory (which
// is exactly the single-segment case, NPts == 2).
