...
- **Focus**: Robotics/attitude control with small matrices (≤4×4), extensive compile-time verification, and embedded suitability
...
- **Constexpr-first design**: All algorithms must work at compile-time; prefer `consteval` for design functions
...
- **Numerical precision**: Maintain ~1e-6 accuracy; use `doctest::Approx().epsilon(1e-6)` in tests
