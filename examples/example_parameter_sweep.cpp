...
int main() {
    std::cout << "=== Parameter Sweep Plotter ===\n\n";
    std::cout << "Closed-loop position control system with variable load mass\n";
    std::cout << "PID controller tuned for nominal mass of 1.0 kg\n\n";

    // Get mass values from user
    std::cout << "Enter mass values (in kg) separated by spaces.\n";
    std::cout << "Example: 0.3 0.5 0.7 1.0 1.5 2.5 5.0 10.0\n";
    std::cout << "Mass values: ";
    ...
    while (iss >> value) {
        if (value > 0) {
            ...
            std::cout << "Ignoring invalid mass: " << value << "\n";
        }
    }

    if (masses.empty()) {
        std::cout << "No valid masses provided. Using default values.\n";
        masses = {0.3, 0.5, 0.7, 1.0, 1.5, 2.5, 5.0, 10.0};
    }

    const size_t num_params = masses.size();
    std::cout << "\nGenerating sweep for " << num_params << " mass values...\n";
    SweepData data;
    ...
    for (size_t i = 0; i < num_params; ++i) {
        futures.push_back(std::async(std::launch::async, [&, i]() {
            ...
            std::cout << fmt::format("  m = {:5.2f} kg | Points: {:>4} | Bode: {:>8.3f} ms | Total: {:>8.3f} ms\n",
                                     mass,
